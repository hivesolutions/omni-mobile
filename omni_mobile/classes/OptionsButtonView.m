...
- (void)drawRect:(CGRect)rect {
    ...
    if(self.widthBorder) {
        ...
        const CGColorRef grayColor = [[UIColor colorWithRed:0.27 green:0.27 blue:0.27 alpha:1.0] CGColor];
        ...
        CGPathAddLineToPoint(path, NULL, 0, self.frame.size.height - 10);
        CGPathCloseSubpath(path);

        // creates the cell's border
        CGMutablePathRef path2 = CGPathCreateMutable();
        CGPathMoveToPoint(path2, NULL, self.frame.size.width, 0);
        CGPathAddLineToPoint(path2, NULL, self.frame.size.width, self.frame.size.height - 10);
